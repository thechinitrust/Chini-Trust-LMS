...
export function CourseThumbnailUpload({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      notify.error("Couldn't upload image", error instanceof Error ? error.message : undefined);
    } finally {
  ...
  return (
    <div>
      <Label>
        Course banner {required && <span className="text-destructive">*</span>}
      </Label>
      <div className="mt-1.5 flex flex-col gap-3 sm:flex-row sm:items-start">
        <div
          className={cn(
            "relative aspect-[16/10] w-full shrink-0 overflow-hidden rounded-xl border bg-muted sm:w-56",
            value ? "border-border" : "border-dashed border-border"
          )}
        >
          {value ? (
            <Image src={value} alt="" fill className="object-cover" sizes="224px" />
          ) : (
        ...
        <div className="space-y-2">
          ...
          <p className="max-w-xs text-xs text-muted-foreground">
            Shown as the course banner on the Learn page. JPG, PNG or WEBP, up to 500KB — a 16:10
