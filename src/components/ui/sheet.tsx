...
const sheetVariants = cva(
  "fixed z-50 gap-4 border-border bg-card p-6 shadow-soft-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=closed]:animate-out data-[state=open]:animate-in",
  {
    variants: {
      side: {
  ...
  ({ side = "right", className, children, ...props }, ref) => (
    <SheetPortal>
      ...
      <DialogPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        <DialogPrimitive.Close className="absolute top-5 right-5 rounded-full p-1.5 text-muted-foreground opacity-70 transition-all hover:bg-muted hover:opacity-100">
