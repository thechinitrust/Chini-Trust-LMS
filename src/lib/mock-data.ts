...
/**
 * Realistic sample data standing in for Supabase queries. Every helper below
 * (getCourseBySlug, getEnrollmentsForUser, ...) is written the way a data
 * access layer over Supabase would be, so swapping the array lookups for
 * `supabase.from(...).select(...)` calls later is a contained change.
 */
...
export const CURRENT_ADMIN_ID = "profile-admin";
...
export const mockProfiles: Profile[] = [
  ...
  },
  {
    ...
    email: "morgan.blake@chinitrust.org",
    role: "admin",
    avatarUrl: "",
...
export const mockModules: Module[] = [
  {
    ...
    order: 1,
    lessonIds: ["lesson-autism-1-1", "lesson-autism-1-2"],
  },
  {
    id: "mod-autism-2",
    courseId: "course-autism",
    ...
    order: 2,
    lessonIds: ["lesson-autism-2-1", "lesson-autism-2-2"],
    quizId: "quiz-autism",
  },
  {
    id: "mod-adhd-1",
    ...
    order: 1,
    lessonIds: ["lesson-adhd-1-1", "lesson-adhd-1-2"],
  },
  {
    id: "mod-adhd-2",
    courseId: "course-adhd",
    ...
    order: 2,
    lessonIds: ["lesson-adhd-2-1", "lesson-adhd-2-2"],
    quizId: "quiz-adhd",
  },
  {
    id: "mod-dyslexia-1",
    ...
    order: 1,
    lessonIds: ["lesson-dyslexia-1-1", "lesson-dyslexia-1-2"],
  },
  {
    id: "mod-dyslexia-2",
    courseId: "course-dyslexia",
  ...
  },
  {
    ...
    order: 1,
    lessonIds: ["lesson-workplace-1-1", "lesson-workplace-1-2"],
...
export const mockQuizzes: Quiz[] = [
  {
    ...
    passThreshold: 70,
    questionIds: ["q-autism-1", "q-autism-2", "q-autism-3"],
  },
  {
    id: "quiz-adhd",
    moduleId: "mod-adhd-2",
    ...
    passThreshold: 70,
    questionIds: ["q-adhd-1", "q-adhd-2", "q-adhd-3"],
  },
  {
    id: "quiz-dyslexia",
    moduleId: "mod-dyslexia-2",
    ...
    passThreshold: 70,
    questionIds: ["q-dyslexia-1", "q-dyslexia-2"],
  },
  {
    id: "quiz-workplace",
    moduleId: "mod-workplace-1",
    ...
    passThreshold: 70,
    questionIds: ["q-workplace-1", "q-workplace-2"],
  },
];
...
// ---------------------------------------------------------------------------
// Lookup helpers — written like a data-access layer so swapping array finds
// for Supabase queries later touches only this file, not the pages/components
// that call them.
// ---------------------------------------------------------------------------
...
export function getLessonsForCourse(courseId: string): Lesson[] {
  return mockLessons
    ...
    .sort((a, b) => {
      const ma = mockModules.find((m) => m.id === a.moduleId)?.order ?? 0;
      const mb = mockModules.find((m) => m.id === b.moduleId)?.order ?? 0;
      return ma - mb || a.order - b.order;
    });
}

export function getResourcesForCourse(courseId: string): Resource[] {
  return mockResources.filter((r) => r.courseId === courseId);
}
...
export function getQuizForModule(moduleId: string): Quiz | undefined {
  return mockQuizzes.find((q) => q.moduleId === moduleId);
}

export function getQuestionsForQuiz(quizId: string): QuizQuestion[] {
  return mockQuizQuestions.filter((q) => q.quizId === quizId).sort((a, b) => a.order - b.order);
}
...
export function getProgressForUser(userId: string): LessonProgress[] {
  return mockLessonProgress.filter((p) => p.userId === userId);
}

export function getProgressForLesson(userId: string, lessonId: string): LessonProgress | undefined {
  return mockLessonProgress.find((p) => p.userId === userId && p.lessonId === lessonId);
}

export function getCourseCompletionPercent(userId: string, courseId: string): number {
...
export function getQuizAttemptsForUser(userId: string): QuizAttempt[] {
  return mockQuizAttempts.filter((a) => a.userId === userId);
}

export function getLatestQuizAttempt(userId: string, quizId: string): QuizAttempt | undefined {
...
export function getNextLesson(lessonId: string): Lesson | undefined {
  ...
  const courseLessons = getLessonsForCourse(current.courseId);
